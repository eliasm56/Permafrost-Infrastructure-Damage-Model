...
UPDATED (FULL POOLING + NO MIXED-SIGN CLASSES):
  - FULL POOLING:
      ...
      No intermediate "mean across hazard maps within each draw".

      STAT = percentile_{(hazard_map, draw)}(dam_cost)

  - NO MIXED-SIGN CLASSES:
      Classification is split at 0 so no legend class ever spans negative→positive.
      Negative and positive values are classified separately (Jenks) and stitched
      with 0 as an explicit boundary.
