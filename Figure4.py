...
This version:
- Keeps OAT figures TOTAL-only (no occupancy needed for OAT modes)
- Adds occupancy-split (Residential / Non-residential) for the THREE MAIN figures ONLY (1–3),
  using the ALL-mode *MC_country_draws.csv if it contains:
    dam_cost_res_usd2024 and dam_cost_nonres_usd2024
- Colors band lines by (region × occupancy) and adds legends
- DOES NOT color violin plots (default violin styling)

UPDATED (requested):
- Replace many OAT attribution outputs with ONE 4-panel figure:
...
Author: Elias Manos
Edits: occupancy-split + colored bands/legend for main bands figure only
Edits: extra space between suptitle and plots (all figures)
Edits: OAT attribution consolidated into a single 4-panel figure (width-based)
"""
...
def main():
    ...
    # 5) NEW: single, consolidated 4-panel OAT attribution figure (width-based)
